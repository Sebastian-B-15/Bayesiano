...
$$p(\theta) = p_0 \cdot I_{(\theta = \theta_0)} + (1 - p_0) \cdot p'(\theta) \cdot I_{(\theta \neq \theta_0)} *$$
Donde $p'(\theta)$ es la fdp original de $\theta$ a priori/posteriori. Como $P(\theta = \theta_0) = p_0$, entonces $P(\theta \neq \theta_0) = 1 - p_0$. De esta forma, escalamos la fdp original por $(1 - p_0)$ cuando $\theta$ es diferente de $\theta_0$. Esta v.a. ahora tiene una parte continua y otra discreta, luego si se integra la fdp en el dominio continuo y se suma la fdp el dominio discreto, el total es $1$.
...
Esto se demuestra usando la fdp ajustada anteriormente para $\theta$.